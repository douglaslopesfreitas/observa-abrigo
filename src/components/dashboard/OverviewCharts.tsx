...
// ✅ 1. Definimos a prop onSelect para navegação
export function OverviewCharts({ onSelect }: { onSelect?: (id: string) => void }) {
  ...
  useEffect(() => {
    getIndicadorSheet("faixa-etaria")
      .then((d) => {
        ...
        const idxFonte = headersNorm.indexOf("fonte");
        const hasFonteColumn = idxFonte >= 0;
        let idxCat = headersNorm.indexOf("categoria") >= 0 ? headersNorm.indexOf("categoria") : headersNorm.indexOf("faixa_etaria");
        if (idxCat < 0) idxCat = detectCategoryFallback(headersNorm);
        ...
        const filtered = parsed.filter((x) => isRJ(x.territorio) && shouldKeepByFonte(x.fonte, hasFonteColumn));
        const dates = Array.from(new Set(filtered.map((x) => x.data).filter(Boolean))).sort();
        const last = dates[dates.length - 1];
        if (!last) { setAgeData([]); return; }
        const rowsLast = filtered.filter((r) => r.data === last && r.categoria && typeof r.valor === "number" && r.valor > 0);
        const total = rowsLast.reduce((acc, r) => acc + (r.valor as number), 0);
        const mapped = rowsLast.map((r) => ({ name: r.categoria, value: ((r.valor as number) / total) * 100 })).sort((a, b) => b.value - a.value);
        setAgeData(mapped);
      })
      .catch(() => setAgeData([]));
  }, []);

  // ===== Recorte racial (aba raça) =====
  useEffect(() => {
    getIndicadorSheet("raça")
      .then((d) => {
        const values: any[][] = d.values || [];
        if (values.length < 2) { setRacaData([]); return; }
        const headersNorm = (values[0] || []).map((h) => normTxt(h));
        const body = values.slice(1);
        const idxTerr = headersNorm.indexOf("territorio");
        const idxData = headersNorm.indexOf("data");
        ...
        const idxFonte = headersNorm.indexOf("fonte");
        const hasFonteColumn = idxFonte >= 0;
        let idxCat = headersNorm.indexOf("raca") >= 0 ? headersNorm.indexOf("raca") : headersNorm.indexOf("raça");
        if (idxCat < 0) idxCat = detectCategoryFallback(headersNorm);

        if (idxTerr < 0 || idxData < 0 || idxVal < 0 || idxCat < 0) { setRacaData([]); return; }
        let lastDateAny = "";
        const parsed: RowParsed[] = body.map((r) => {
          const rawDate = String(r[idxData] ?? "").trim();
          if (rawDate) lastDateAny = rawDate;
        ...
        const mapped = rowsLast.map((r) => ({ name: r.categoria, value: totalRaca > 0 ? ((r.valor || 0) / totalRaca) * 100 : 0 })).sort((a, b) => b.value - a.value);
        setRacaData(mapped);
      })
      .catch(() => setRacaData([]));
  }, []);

  // ===== Maiores Necessidades (aba doacao) =====
  useEffect(() => {
    getIndicadorSheet("doacao")
      .then((d) => {
        const values: any[][] = d.values || [];
        if (values.length < 2) { setNeedsData([]); return; }
        const headersNorm = (values[0] || []).map((h) => normTxt(h));
        const body = values.slice(1);
        const idxTerr = headersNorm.indexOf("territorio");
        const idxData = headersNorm.indexOf("data");
        ...
        const idxFonte = headersNorm.indexOf("fonte");
        const hasFonteColumn = idxFonte >= 0;
        let idxCat = headersNorm.indexOf("categoria") >= 0 ? headersNorm.indexOf("categoria") : headersNorm.indexOf("necessidade");
        if (idxCat < 0) idxCat = detectCategoryFallback(headersNorm);

        if (idxTerr < 0 || idxData < 0 || idxVal < 0 || idxCat < 0) { setNeedsData([]); return; }
        let lastDateAny = "";
        const parsed: RowParsed[] = body.map((r) => {
          const rawDate = String(r[idxData] ?? "").trim();
          if (rawDate) lastDateAny = rawDate;
        ...
        const dates = Array.from(new Set(filtered.map((x) => x.data).filter(Boolean))).sort();
        const last = dates[dates.length - 1];
        if (!last) { setNeedsData([]); return; }
        const rowsLast = filtered.filter((r) => r.data === last && r.categoria && typeof r.valor === "number" && r.valor > 0);
        const mapped = rowsLast.map((r) => ({ name: r.categoria, value: ((r.valor as number) / NEEDS_TOTAL) * 100 })).filter((x) => Number.isFinite(x.value) && x.value > 0).sort((a, b) => b.value - a.value);
        setNeedsData(mapped);
      })
      .catch(() => setNeedsData([]));
  }, []);
  ...
  // ✅ Classe CSS de clique para reuso
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      {evolutionData.length === 0 ? (
        ...
        <div 
          onClick={() => onSelect?.("acolhidos")} // ✅ ID do catálogo
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={evolutionData}>
                ...
                <XAxis dataKey="name" tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} axisLine={{ stroke: "hsl(var(--border))" }} tickFormatter={evolutionTickFormatter} />
                <YAxis tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} axisLine={{ stroke: "hsl(var(--border))" }} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} />
                <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "8px" }} labelFormatter={(label) => `Referência: ${formatDateBR(label)}`} formatter={(value: number) => [value.toLocaleString("pt-BR"), "Acolhidos"]} />
                <Line type="monotone" dataKey="value" stroke={PRIMARY_COLOR} strokeWidth={2} dot={{ fill: PRIMARY_COLOR, strokeWidth: 2, r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {ageData.length === 0 ? (
        ...
        <div 
          onClick={() => onSelect?.("faixa-etaria")} // ✅ ID do catálogo
          className={clickableClass} 
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={ageData} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis type="number" domain={[0, 100]} tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} tickFormatter={(v) => `${Number(v).toFixed(0)}%`} />
                <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }} width={130} />
                <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "8px" }} formatter={(value: number) => [`${value.toFixed(1).replace(".", ",")}%`, "Percentual"]} />
      ...
      {racaData.length === 0 ? (
        ...
        <div 
          onClick={() => onSelect?.("raça")} // ✅ ID do catálogo
          className={clickableClass} 
        ...
        >
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={racaData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value" isAnimationActive={true} animationDuration={1000} label={({ name, value }) => `${name}: ${value.toFixed(1).replace(".", ",")}%`}>
      ...
      {/* 4. Maiores Necessidades */}
      {needsData.length === 0 ? (
        <EmptyState title="Maiores Necessidades" />
      ) : (
        <div 
          onClick={() => onSelect?.("doacao")} // ✅ ID do catálogo
          className={clickableClass} 
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={needsData} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis type="number" domain={[0, 100]} tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} tickFormatter={(v) => `${Number(v).toFixed(0)}%`} />
                <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }} width={200} interval={0} />
                <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "8px" }} formatter={(value: number) => [`${value.toFixed(1).replace(".", ",")}%`, "Percentual"]} />
                <Bar dataKey="value" fill={PRIMARY_COLOR} radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
